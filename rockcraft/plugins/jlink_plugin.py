...
from typing import Any, Dict, Literal, List, Set, cast
...
class JLinkPlugin(Plugin):
    ...
    def get_build_commands(self) -> List[str]:
        ...
        if hasattr(options, "source") and isinstance(options.source, str):
            commands.append("chisel cut --release `pwd` --root ${CRAFT_PART_INSTALL} base-files_base openjdk-"+str(options.jlink_java_version)+"-jre-headless_headless")
        else:
            commands.append("chisel cut --root ${CRAFT_PART_INSTALL} base-files_base openjdk-" + str(options.jlink_java_version) + "-jre-headless_headless")
        ...
        commands.append("if [ \"x${PROCESS_JARS}\" == \"x\" ]; then deps=$(jdeps --class-path=${cpath} -q --recursive  --ignore-missing-deps --print-module-deps --multi-release 21 ${PROCESS_JARS}; else deps=java.base; fi)")
